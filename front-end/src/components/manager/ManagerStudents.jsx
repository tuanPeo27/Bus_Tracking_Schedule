...
import { createStudent, deleteStudent, getAllStudent, getInfoRoute, updateStudent, getAllParent, getAllRoute, getBusStopByRouteId } from "../../service/adminService";
...
export default function ManagerStudents() {
  ...
  const [listBusStop, setListBusStop] = useState([]);
  ...
  const fetchBusStops = async (idRoute) => {
    try {
      const res = await getBusStopByRouteId(idRoute);
      const data = res?.data;
      if (data && data.EC === 0 && Array.isArray(data.DT)) {
        setListBusStop(data.DT);
        setFilteredBusStops(data.DT); // <--- filter đúng dữ liệu mới trả về
      } else {
        console.error("Lỗi khi lấy dữ liệu trạm dừng:", errorMessage);
      }
    } catch (error) {
      console.error("Lỗi mạng khi lấy trạm dừng:", error.message);
    }
  };
      ...
      if (res?.data?.EC === 0) {
        const infoStudent = await Promise.all(
          dataStudent.map(async (item) => {
            ...
            const busStopInfo = await getBusStopByRouteId(item.route_id);
            return { ...item, routeInfo, parentInfo, busStopInfo };
          })
        );
  ...
  const handleAddStudent = async () => {
    ...
    if (
      ...
      !newStudent.route_id ||
      !newStudent.pickup_point_id ||
      !newStudent.dropoff_point_id
    ) {
    ...
    if (/\d/.test(newStudent.school)) {
      showError("Vui lòng nhập trường hợp lệ!!");
      return;
    }
  ...
  const handleUpdateStudent = async () => {
    ...
    if (editStudent.age < 6 || editStudent.age > 99) {
      showError("Vui lòng nhập tuổi hợp lệ!!");
      return;
    }
    if (/\d/.test(editStudent.name)) {
      showError("Vui lòng nhập họ và tên hợp lệ!!");
      return;
    }
    if (/\d/.test(editStudent.school)) {
      showError("Vui lòng nhập trường hợp lệ!!");
      return;
    }
    try {
                  ...
                  <div className="space-y-4">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => {
                        ...
                        // GỌI API → tự set filtered bus stop
                    ...
                    <Select
                      ...
                      value={newStudent.pickup_point_id}
                      onValueChange={(value) =>
                        setNewStudent({ ...newStudent, pickup_point_id: value })
                      }
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Chọn trạm đón" />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <Select
                      disabled={!newStudent.route_id}
                      value={newStudent.dropoff_point_id}
                      onValueChange={(value) =>
                        setNewStudent({ ...newStudent, dropoff_point_id: value })
                      }
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Chọn trạm trả" />
                      </SelectTrigger>
                      <SelectContent>
                        {filteredBusStops.map((stop) => (
                          <SelectItem key={stop.id} value={stop.id}>
              ...
              <Table className="min-w-full">
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Điểm xuống</TableHead>
                      ...
                      <TableRow key={student?.id}>
                        ...
                        <TableCell>
                          <div>
                            <p className="font-medium">
                              {
                                student?.busStopInfo?.data?.DT?.find(
                                  (s) => Number(s.id) === Number(student?.pickup_point_id)
                                )?.name || "Không có"
                              }
                            </p>
                        ...
                        <TableCell>
                          <div>
                            <p className="font-medium">
                              {
                                student?.busStopInfo?.data?.DT?.find(
                                  (s) => Number(s.id) === Number(student?.dropoff_point_id)
                                )?.name || "Không có"
                              }
                            </p>
                          </div>
